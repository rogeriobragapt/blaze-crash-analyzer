...
const MelhoresPadroesIA24h = () => {
  ...
  const fetchAiPatterns = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Falha ao buscar melhores padrões da IA (24h)");
      }
      ...
      if (err instanceof Error) {
        ...
        setError("Ocorreu um erro desconhecido ao buscar os melhores padrões da IA (24h).");
      }
      console.error(err);
    }
    setIsLoading(false);
  ...
  }, [session, fetchAiPatterns]);

  return (
    <div className="bg-gray-900 p-6 rounded-lg shadow-lg border border-gray-700">
      <h2 className="text-xl font-semibold mb-4 text-purple-300">Melhores Padrões da IA (Últimas 24h)</h2>
      ...
      <div className="bg-gray-800 p-4 rounded min-h-[100px] max-h-[300px] overflow-y-auto border border-gray-700">
        ...
        {!isLoading && patterns.length === 0 && (
          <p className="text-gray-400 text-center">Nenhum padrão da IA relevante nas últimas 24h. Adicione ao catalogador para a IA aprender.</p>
        )}
        {!isLoading && patterns.length > 0 && (
            ...
            <tbody>
              {patterns.map((p) => (
                ...
                >
                  ...
                  <td className="py-2 text-center text-green-400">12</td>
                  <td className="py-2 text-center text-gray-300">13</td>
                  <td className="py-2 text-center text-yellow-400">92%</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default MelhoresPadroesIA24h;
